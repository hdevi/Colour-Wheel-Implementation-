//////////////////////////////////////////////////////////////
// timer_int.c - Contains Source code for Timers and System Initialization
//
...
// Description:
// ------------
// Implemented PWM using Software Detection inside FIT Handler.
// AXI Timer Initialization Function is used to generate the Signal which is given to Nexsys4IO module.
// do_init() is used initialized the system and peripherals connected to the board.
// Header files peripherals.h and timer_init.h are included in these files.
////////////////////////////////////////////////////////////////
...
void FIT_Handler(void)
{
	...
	if((gpio_in & 0x0001) == 1)
	{
		if(green_high_count == 1 && green_low_count == 1)
		{
			...
			final_green_low= temp_green_low;
...
int do_init()
{
	...
	if (status != XST_SUCCESS)
	{
		xil_printf("Interrupt Controller failed 0");
		return XST_FAILURE;
	}
	...
	if (status != XST_SUCCESS)
	{
		xil_printf("Fixed Interval Timer failed 0");
		return XST_FAILURE;
...
int AXI_Timer_initialize(void){
	...
	XTmrCtr_SetLoadReg(AXI_TIMER_BASEADDR, TmrCtrNumber, 250);
